---
title: How Do I...?
description: Common tasks for TabulaKit site owners — just ask Claude Code in plain English
---

:::note[Migration status]
This page was migrated from the v1 Docsify source at [`heatherstoneio/tabulakit/site/how-do-i.md`](https://github.com/heatherstoneio/tabulakit/blob/main/site/how-do-i.md) as a smoke-test for OPORD 202604-019 Phase 1.
:::
...
Claude Code will create the file in `src/content/docs/`, add it to the sidebar in `astro.config.mjs`, and even draft some starter content if you describe what the page is about.
...
In v2, the site title is exposed as a single config value (either `TABULAKIT_TITLE` env var or the `SITE_TITLE` constant in `astro.config.mjs`).

---

## How do I deploy my changes?

If you're using **GitHub Pages** (default in v2) or **Netlify**, changes deploy automatically when you push. Just tell Claude Code:
...
Put the image file in `src/assets/` (you can drag and drop it into VS Code), then tell Claude Code:

- *"Add the logo.png image to the home page"*
- *"Put a banner image at the top of the About page"*

Images referenced from markdown are automatically optimized by Astro at build time.
...
The v2 default (GitHub Pages) is public. For access-controlled hosting, see the [v1 Firebase guide](https://github.com/heatherstoneio/tabulakit/blob/main/site/deploy-firebase.md) — the v2 approach is being evaluated in OPORD 202604-021.
...
Just ask. Claude Code understands your entire project and can handle most requests. The worst that happens is it tells you something isn't possible and suggests an alternative.
